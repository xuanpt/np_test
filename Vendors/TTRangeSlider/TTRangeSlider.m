...
//expand the touch area of the handle by this much (negative values increase size) so that you don't have to touch right on the handle to activate it.
const int HANDLE_TOUCH_AREA_EXPANSION = 0;
...
@property (nonatomic, strong) CALayer *sliderLeftLine;
@property (nonatomic, strong) CALayer *sliderRightLine;
...
static const CGFloat kLabelsFontSize = 16;
...
- (void)initialiseControl {
    ...
    self.sliderLeftLine = [CALayer layer];
    self.sliderLeftLine.backgroundColor = UIColorFromRGBValues(202, 202, 202).CGColor;;
    [self.layer addSublayer:self.sliderLeftLine];
    
    
    self.sliderRightLine = [CALayer layer];
    self.sliderRightLine.backgroundColor = UIColorFromRGBValues(202, 202, 202).CGColor;;
    [self.layer addSublayer:self.sliderRightLine];
    

    //draw the minimum slider handle
    self.leftHandle = [CALayer layer];
    self.leftHandle.cornerRadius = 3.0f;
    ...
    self.rightHandle.cornerRadius = 3.0f;
...
- (void)layoutSubviews {
    ...
    self.sliderLeftLine.frame = CGRectMake(10, lineLeftSide.y, self.leftHandle.position.x, 10);
    self.sliderRightLine.frame = CGRectMake(self.rightHandle.position.x, lineLeftSide.y, currentFrame.size.width - self.rightHandle.position.x - 10, 10);
}
...
- (void)tintColorDidChange {
    ...
    self.leftHandle.backgroundColor = DefaultButtonColor.CGColor;
    self.rightHandle.backgroundColor = DefaultButtonColor.CGColor;
